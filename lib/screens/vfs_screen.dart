...
class _VfsScreenState extends State<VfsScreen> {
          ...
          if (vfs.currentPath.startsWith('/tools'))
            Padding(
              ...
              child: TextButton.icon(
                onPressed: () => _showToolMarketplace(context),
  ...
  void _showToolMarketplace(BuildContext context) {
    showModalBottomSheet(
      context: context,
      ...
      builder: (ctx) {
        ...
        return _MarketplaceSheet(registry: registry, repo: repo);
      },
    );
  }
...
class _MarketplaceSheet extends StatefulWidget {
  ...
  final ToolRepositoryService repo;
  const _MarketplaceSheet(
      {required this.registry, required this.repo});
...
class _MarketplaceSheetState extends State<_MarketplaceSheet> {
      ...
      builder: (_, scrollCtrl) => Column(
        children: [
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 20),
            child: Text(
              'Install tools to give the AI agent more capabilities',
              style: TextStyle(
                color: AppColors.textSecondary(context),
                fontSize: 13,
              ),
          ...
          Expanded(
            child: ListenableBuilder(
              ...
              builder: (context2, _) => ListView.builder(
                controller: scrollCtrl,
                padding: const EdgeInsets.symmetric(horizontal: 12),
                itemCount: widget.registry.tools.length,
                itemBuilder: (_, i) {
                  final tool = widget.registry.tools[i];
                  final installed = widget.registry.isInstalled(tool.name);
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      trailing: installed
                          ...
                          : TextButton(
                              onPressed: () {
                                _installTool(context, tool.name);
                              },
                              child: Text('Install',
                                  style: TextStyle(
                                      color: AppColors.primary,
  ...
  Future<void> _refreshTools() async {
    ...
    await widget.repo.loadIntoRegistry(widget.registry);
